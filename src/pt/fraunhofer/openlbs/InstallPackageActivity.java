...
public class InstallPackageActivity extends Activity {
	...
	private Runnable fetchContent = new Runnable() {
		...
		public void run() {
			...
			try {
				...
				Bundle b = new Bundle();
				b.putString(DOWNLOAD_INFORMATION_TYPE, DOWNLOAD_SIZE_NOTIFICATION);
	...
	private Runnable unzipContents = new Runnable() {
		...
		public void run() {
			...
			try {
				...
				Bundle bf = new Bundle();
				bf.putString(UNZIP_INFORMATION_TYPE, UNZIP_COMPLETION_NOTIFICATION);
				msgf.setData(bf);
	...
	final Handler downloadHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			String messageType = msg.getData().getString(DOWNLOAD_INFORMATION_TYPE);
			
			if (messageType.equals(DOWNLOAD_SIZE_NOTIFICATION)) {
				downloadDialog.setMax(msg.getData().getInt(DOWNLOAD_SIZE_NOTIFICATION));
			}

			if (messageType.equals(DOWNLOAD_PROGRESS_NOTIFICATION)) {
				downloadDialog.setProgress(msg.getData().getInt(DOWNLOAD_PROGRESS_NOTIFICATION));
			}
			...
			if (messageType.equals(DOWNLOAD_FINISHED)) {
				...
				Thread unzipThread = new Thread(null, unzipContents, "ContentUnzip");
				unzipThread.start();
			}
	...
	final Handler unzipHandler = new Handler() {
		...
		@Override
		public void handleMessage(Message msg){
			String messageType = msg.getData().getString(UNZIP_INFORMATION_TYPE);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		thisPackageContentURL = CONTENT_FILE_URL.replace("#", thisPackage.getContent_file_name());
		
		Log.d(TAG, "Content file: " + thisPackage.getContent_file_name());
		
		if (INETTools.hasInternet(this)) {
			Thread downloadThread = new Thread(null, fetchContent, "ContentFetch");
	...
	protected Dialog onCreateDialog(int id) {
	    ...
	    case DIALOG_DOWNLOAD_PROGRESS:
	    	...
	    	downloadDialog.setMessage("Downloading contents.\nHold on a jiff...");
